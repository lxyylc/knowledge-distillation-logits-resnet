...
#pruned_model = deepcopy(original_model).to(device)
for m in pruned_model.modules():
    ...
    elif isinstance(m, nn.Linear) and layer_id == len(masks):
        #最后一个卷积层后面跟着的那个全连接层，只剪输入通道
        m.weight.data = m.weight.data[:, in_mask].clone()
        m.in_features = len(in_mask)
        layer_id+=1
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.1)  # 学习率调度器
